...
class BranchAndBoundV1(BranchAndBoundGeneric):
    ...
    def run(self, *args, X_trn, return_objective=False):
        ...
        while not self.queue.is_empty:
            queue_item = self.queue.pop()
            yield from self._loop(*queue_item, X_trn, equivalence_classes, return_objective=return_objective)



    def _loop(
        self, parent_node: Node, parent_not_captured: np.ndarray, X_trn: np.array, equivalence_classes: dict, return_objective=False
        ):
        """
        check one node in the search tree, update the queue, and yield feasible solution if exists
        ...
        for rule in self.rules:
            if rule.id > parent_node.rule_id:
                ...
                flag_rule_set_size = rule_set_size_bound_with_default( parent_node, self.lmbd, self.ub ) # if true, we prune

                flag_equivalent_classes =  equivalent_points_bounds(lb, self.lmbd, self.ub, 
                             parent_not_captured, X_trn, equivalence_classes) # if true, we prune

                if lb <= self.ub and not flag_rule_set_size and not flag_equivalent_classes:
                    fn_fraction, not_captured = incremental_update_obj(
                        parent_not_captured, captured, self.y
                    )
                    obj = lb + fn_fraction
