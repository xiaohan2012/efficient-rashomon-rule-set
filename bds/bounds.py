...
def rule_set_size_bound(parent_node: Node, lmbd: float, current_optimal_objective: float, alpha: float): 
    '''
    Simple pruning condition according to solely rule-set size. 
...
def bound_on_false_positives(lb: float, parent_node: Node, lmbd: float, current_optimal_objective: float, alpha: float):
    '''
    ...
    return lb > current_optimal_objective + alpha - lmbd*(  len( parent_node.get_ruleset_ids() ) - 1  )
