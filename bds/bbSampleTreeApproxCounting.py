...
class BranchAndBoundGeneric:
    ...
    def runST(self, return_objective=False, **kwargs) -> Iterable:
        ...
        pseudosolutions = [
            ({0}, (self.tree._root, not_captured_root), self.tree._root.objective)
        ]  # empty -
        ...
        for h in range(L):
            previous_pseudosolutions = copy.deepcopy(pseudosolutions)

            pseudosolutions, this_ratio = self.generate_single_level(pseudosolutions)
        ...
        Z /= this_ratio
        Z *= len(last_level_pseudosolutions) / len(previous_pseudosolutions)
    ...
    def generate_single_level(self, pseudosolutions) -> Iterable:
        ...
        ratio = (
            len(all_solutions) / n_samples
        )  # len(pseudosolutions) #len(pseudosolutions)
        return all_solutions, ratio
...
class BranchAndBoundNaive(BranchAndBoundGeneric):
    ...
    def _create_new_node_and_add_to_tree(
        self, rule: Rule, lb: mpfr, obj: mpfr, captured: mpz, parent_node: NodeST
    ) -> NodeST:
    ...
    # @profile
    def _loopST(
        self, parent_node: NodeST, parent_not_captured: mpz, return_objective=False
    ):
        """
        ...
        for rule in self.rules[self.current_length : (self.current_length + self.l)]:
            ...
            this_rule_set = "-".join(
                sorted(list(map(str, parent_node.get_ruleset_ids().union({rule.id}))))
            )
